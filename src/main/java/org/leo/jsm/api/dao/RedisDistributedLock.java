...
public class RedisDistributedLock {
    ...
    private final Logger logger = Logger.getLogger(RedisDistributedLock.class);

    public boolean setLock(final String key, final long expire) {
        try {
 ...
 * Example: 
 * 
    String key = "lock.apiname.userid." + userId;
    Boolean tryGetLock = redisDistributedLock.setLock(key, 5000);
    String requestId = null;

    // 没有获取到锁
    if (!tryGetLock) {
        return message(Response.Status.BAD_REQUEST, 930021, null);
    } else {
